...
function AuthPage({ mode, onSwitchToLogin, onSwitchToSignup, onForgotPassword, onAuthenticated }) {
  ...
  const heading = isLogin ? 'Sign in to WasteWise' : 'Create your account'
  ...
  const switchCopy = isLogin ? 'New to WasteWise?' : 'Already have an account?'
  const switchAction = isLogin ? onSwitchToSignup : onSwitchToLogin
  ...
  const handleSubmit = async (event) => {
    ...
    if (!isLogin && signupForm.password !== signupForm.confirmPassword) {
      setLoading(false)
      setFeedback({
        type: 'error',
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data?.detail ?? `Unable to ${isLogin ? 'sign in' : 'sign up'} right now.`,
        )
      }
      ...
      if (isLogin) {
        ...
        storage.setItem('wastewise_access_token', data.access_token)
        storage.setItem('wastewise_refresh_token', data.refresh_token)
        onAuthenticated?.()
      } else {
        window.setTimeout(() => onSwitchToLogin?.(), 2000)
      }
    } catch (error) {
      setFeedback({
        type: 'error',
      ...
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden bg-gradient-to-br from-green-700 via-green-600 to-emerald-500 p-12 flex-col justify-between text-white">
        ...
        <div className="relative max-w-lg">
          ...
          <p className="mt-6 text-lg leading-relaxed text-green-50">
            Turn everyday sustainable actions into measurable impact, Green Points,
            rewards, and a better future for our planet.
          </p>

          <div className="mt-10 flex gap-8">
            <div>
              <p className="text-2xl font-bold">01</p>
              <p className="mt-1 text-sm text-green-100">Take Action</p>
            </div>
            <div>
              <p className="text-2xl font-bold">02</p>
              <p className="mt-1 text-sm text-green-100">Earn Rewards</p>
            </div>
            <div>
              <p className="text-2xl font-bold">03</p>
              <p className="mt-1 text-sm text-green-100">Create Impact</p>
            </div>
          </div>
        </div>
        ...
        <div className="w-full max-w-md">
          ...
          <div className="mb-8">
            <p className="text-sm font-medium text-green-600 mb-2">
              {isLogin ? 'WELCOME BACK' : 'WELCOME TO WASTEWISE'}
            </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              {isLogin ? (
                <div className="mb-3 grid grid-cols-2 rounded-xl bg-green-50 p-1 text-sm font-semibold">
                  <button
                    ...
                    className={`rounded-lg py-2 transition ${loginMethod === 'email' ? 'bg-white text-green-800 shadow-sm' : 'text-green-700/70'}`}
                  >
                    Email
                  </button>
                  <button
                    ...
                    className={`rounded-lg py-2 transition ${loginMethod === 'username' ? 'bg-white text-green-800 shadow-sm' : 'text-green-700/70'}`}
                  >
                    Username
                  </button>
                </div>
              ...
              <label className="block text-sm font-medium text-gray-700 mb-2">
                {isLogin ? (loginMethod === 'email' ? 'Email address' : 'Username') : 'Email address'}
              </label>
              <div className="relative">
                {isLogin && loginMethod === 'username' ? <User
                  size={20}
                  className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400"
                /> : <Mail
                  size={20}
                ...
                />}
                <input
                  type={isLogin && loginMethod === 'username' ? 'text' : 'email'}
                  placeholder={isLogin && loginMethod === 'username' ? 'Enter your username' : 'you@example.com'}
                  value={isLogin ? loginForm.identifier : signupForm.email}
                  onChange={(event) =>
                    isLogin
                      ? updateLoginForm('identifier', event.target.value)
                      : updateSignupForm('email', event.target.value)
                  }
                  required
                  className="w-full rounded-xl border border-gray-200 bg-white py-3.5 pl-12 pr-4 outline-none transition focus:border-green-500 focus:ring-4 focus:ring-green-100"
                />
                ...
                <input
                  ...
                  onChange={(event) =>
                    isLogin
                      ...
                      : updateSignupForm('password', event.target.value)
                  }
                  required
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      updateSignupForm('confirmPassword', event.target.value)
                    }
                    required
                    className="w-full rounded-xl border border-gray-200 bg-white py-3.5 pl-12 pr-4 outline-none transition focus:border-green-500 focus:ring-4 focus:ring-green-100"
                  />
            ...
            {/* {isLogin ? (
              ...
              <label className="flex items-start gap-2 text-sm text-gray-600 cursor-pointer">
                <input
                  ...
                  onChange={(event) =>
                    updateSignupForm('agreeTerms', event.target.checked)
                  }
